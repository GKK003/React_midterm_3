...
function MoviesPage() {
  ...
  const [bookmarks, setBookmarks] = useState(
    data.filter((el) => el.isBookmarked).map((el) => el.title),
  );
  ...
  function HandleBookmark(movieTitle: string) {
    if (bookmarks.includes(movieTitle)) {
      setBookmarks(bookmarks.filter((title) => title !== movieTitle));
    } else {
      setBookmarks([...bookmarks, movieTitle]);
    }
  }
      ...
      <div className="w-[80%] mt-15 flex flex-col items-center justify-center">
        <div className="w-[80%] mt-3 border-b-2 border-transparent focus-within:border-[#5A698F] relative">
          ...
          <input
            ...
            className="h-9 w-full bg-transparent pl-10 text-white placeholder:text-white placeholder:opacity-50 placeholder:text-[15px]"
            ...
            {displayed.map((el) => (
              ...
              >
                <div className="relative rounded-lg overflow-hidden">
                  ...
                  <button
                    onClick={() => HandleBookmark(el.title)}
                  ...
                  >
                    <img
                      src={bookmarks.includes(el.title) ? Bookmarked : Bookmark}
                      alt="bookmark"
                      className="w-3 h-4"
                    />
                  </button>
